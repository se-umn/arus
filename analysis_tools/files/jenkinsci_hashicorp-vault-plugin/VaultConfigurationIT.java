...
public class VaultConfigurationIT {
    ...
    public void setupJenkins() throws IOException {
        ...
        SystemCredentialsProvider.getInstance().setDomainCredentialsMap(Collections.singletonMap(Domain.global(), Arrays.asList(GLOBAL_CREDENTIAL_1, GLOBAL_CREDENTIAL_2)));
    ...
    public void shouldUseGlobalConfiguration() throws Exception {
        ...
        assertThat(vaultBuildWrapper.getConfiguration().getEngineVersion(), is(GLOBAL_ENGINE_VERSION_2));
    ...
    public void shouldDealWithTokenBasedCredential() throws Exception {
        ...
        VaultAccessor mockAccessor = mockVaultAccessor(GLOBAL_ENGINE_VERSION_2);
        vaultBuildWrapper.setVaultAccessor(mockAccessor);
        VaultCredential credential = new VaultTokenCredential(CredentialsScope.GLOBAL, "token-1", "description", Secret.fromString("test-token"));
        SystemCredentialsProvider.getInstance().setDomainCredentialsMap(Collections.singletonMap(Domain.global(), Collections.singletonList(credential)));
        this.project.getBuildWrappersList().add(vaultBuildWrapper);
        VaultConfiguration vaultConfig = new VaultConfiguration();
        vaultConfig.setVaultUrl("http://job-vault-url.com");
