...
public class VaultSSHUserPrivateKeyIT {
    ...
    public void shouldRetrieveCorrectCredentialsFromVault() {
        ...
        //         VaultSSHUserPrivateKey up = mock(VaultSSHUserPrivateKeyImpl.class);
        ...
        //         CredentialsProvider.lookupStores(story.j.jenkins).iterator().next().addCredentials(Domain.global(), up);
        //         WorkflowJob p = story.j.jenkins.createProject(WorkflowJob.class, jobId);
        //         p.setDefinition(new CpsFlowDefinition("" + "node {\n" + " withCredentials([sshUserPrivateKey(credentialsId: '" + credentialsId + "', usernameVariable: 'USERNAME', passphraseVariable: 'PASSPHRASE', keyFileVariable: 'SSH_KEY')]) { " + "      " + getShellString() + " 'echo " + getVariable("USERNAME") + ":" + getVariable("PASSPHRASE") + " > script'\n" + "      " + getShellString() + " '" + getCatString() + " \"" + getVariable("SSH_KEY") + "\" > private_key'\n" + "  }\n" + "}", true));
    ...
    public void shouldRetrieveCorrectCredentialsFromVaultWithCustomKeys() {
        ...
        //     @Override
        //     public void evaluate() throws Throwable {
        //         VaultSSHUserPrivateKeyImpl vup = new VaultSSHUserPrivateKeyImpl(null, credentialsId, "Test Credentials");
        ...
        //         CredentialsProvider.lookupStores(story.j.jenkins).iterator().next().addCredentials(Domain.global(), vup_spy);
        //         WorkflowJob p = story.j.jenkins.createProject(WorkflowJob.class, jobId);
        //         p.setDefinition(new CpsFlowDefinition("" + "node {\n" + " withCredentials([sshUserPrivateKey(credentialsId: '" + credentialsId + "', usernameVariable: 'USERNAME', passphraseVariable: 'PASSPHRASE', keyFileVariable: 'SSH_KEY')]) { " + "      " + getShellString() + " 'echo " + getVariable("USERNAME") + ":" + getVariable("PASSPHRASE") + " > script'\n" + "      " + getShellString() + " '" + getCatString() + " \"" + getVariable("SSH_KEY") + "\" > private_key'\n" + "  }\n" + "}", true));
        //         WorkflowRun b = p.scheduleBuild2(0).waitForStart();
        //         story.j.assertBuildStatus(Result.SUCCESS, story.j.waitForCompletion(b));
    ...
    public void shouldFailIfMissingCredentials() {
        ...
        //     @Override
        //     public void evaluate() throws Throwable {
        //         VaultSSHUserPrivateKeyImpl c = new VaultSSHUserPrivateKeyImpl(null, credentialsId, "Test Credentials");
