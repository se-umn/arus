...
public class OidcIdentityProviderTest extends AbstractOidcTest {
    ...
    public void init() throws Exception {
        ...
        verify(context).redirectTo(ISSUER_URI + "/protocol/openid-connect/auth?response_type=code&client_id=id" + "&redirect_uri=http%3A%2F%2Flocalhost%2Fcallback%2Foidc&scope=openid+email+profile&state=state");
    }
    ...
    private OidcClient newMockClient() {
        ...
        try {
            when(request.toURI()).thenReturn(new URI(ISSUER_URI + "/protocol/openid-connect/auth" + "?response_type=code&client_id=id" + "&redirect_uri=http%3A%2F%2Flocalhost%2Fcallback%2Foidc" + "&scope=openid+email+profile&state=state"));
