...
public class ReadCollectedReposInfo {

  private static final String RESULTS_FILE_NAME = "data/relevant_repos.json";

  public static void main(String args[]){
      ReadCollectedReposInfo rra = new ReadCollectedReposInfo();
      String reposAnalysisFileName = "data/all_repos.json";
  ...
  private void findRelevantProjects(String reposAnalysisFileName){
    try {
      ...
      for(int i=0; i<reposInfoArray.size(); ++i){
        ...
        if(isValidVersionOfMockitoInCommitAnalysis && passedAndNotFlakyInCommitAnalysis>0){
        ...
        else if (isValidVersionOfMockitoInTagAnalysis && passedAndNotFlakyInTagAnalysis>0){
      ...
      saveResults(relevantReposArray);
    }
    catch(Exception e){
  ...
  private void saveResults(JsonArray resultsArray){
    try {
      ...
      FileWriter resultsFileWriter = new FileWriter(ReadCollectedReposInfo.RESULTS_FILE_NAME);
