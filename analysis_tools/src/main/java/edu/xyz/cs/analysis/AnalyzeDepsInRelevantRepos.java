...
import edu.xyz.cs.analysis.model.ExperimentRepoInfo;
...
public class AnalyzeDepsInRelevantRepos {
  ...
  public static void main(String args[]){
    if(args.length!=1){
      System.out.println("usage: ./gradlew clean -PmainClass=edu.xyz.cs.analysis.RunExperiment run --args=\"config_file_name\"");
      System.exit(1);
    }
    try {
  ...
  private void findRelevantProjects(String relevantReposFileName){
    ...
    private void analyzeDepsInRepo(JsonObject repoInfo, FileWriter resultsFileWriter) throws IOException {
      ...
      try {
        ...
        boolean gotDependencyTree = MavenUtils.runMavenDependencyTree(originalPomFile, AnalyzeDepsInRelevantRepos.MAVEN_HOME, "xyz_dependency_tree.xml");
        ...
        if (workingDirectoryInRepo.equals("")) {
          dependencyTreeFileName = currRepoFolderName + File.separator + "xyz_dependency_tree.xml";
        } else {
          dependencyTreeFileName = currRepoFolderName + File.separator + workingDirectoryInRepo + File.separator + "xyz_dependency_tree.xml";
        }
        //junit
